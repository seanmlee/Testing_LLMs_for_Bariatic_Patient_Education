...
run_bootstrap_comparison <- function(data, cond_dougall, cond_other, level = "2", newdata, R = 200) {
  set.seed(123)
  ...
  boot_result <- boot(
    ...
    statistic = function(d, i) {
      ...
      p_dougall <- preds_df[[level]][preds_df$id == cond_dougall]
      p_other   <- preds_df[[level]][preds_df$id == cond_other]
      
      odds_dougall <- p_dougall / (1 - p_dougall)
      odds_other   <- p_other / (1 - p_other)
      
      odds_dougall / odds_other
    },
    R = R
  )
  ...
  point_estimate <- odds_dougall / odds_other
...
pairs_to_compare <- purrr::map(prompt_types, function(pt) {
  ...
  purrr::map(other_models, function(m) {
    list(
      cond_dougall = get_cond_name("DougallGPT", pt),
...
all_results <- purrr::map_df(pairs_to_compare, function(pair) {
  run_bootstrap_comparison(
    data = ai,
    cond_dougall = pair$cond_dougall,
...
write.csv(all_results, "out/posthoc_dougall_vs_others.csv", row.names = FALSE)
