...
# read posthoc -----------------------------------------------------------------
all_results <- read.csv("posthoc_dougall_vs_others.csv", header = TRUE)
...
all_results_plot <- all_results %>%
  ...
  filter(
    !grepl("DocsGPT", condition_2, ignore.case = TRUE)) %>%
...
plot_posthoc_correctness_dougall <- 
  ...
  theme(
    ...
    axis.title.x = element_text(margin = margin(b = 8))
    ) +
...
print(plot_posthoc_correctness_dougall)
